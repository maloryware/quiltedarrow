...
public class ArmorItem extends net.minecraft.item.ArmorItem implements GeoAnimatable, GeoItem {


	private static final Map<ArmorMaterial, StatusEffectInstance> FullKitEffectMap =
		(new ImmutableMap.Builder<ArmorMaterial, StatusEffectInstance>())
			.put(ArmorMaterials.BONE, new StatusEffectInstance(QArrowEffects.STEALTH, 0, 1,
	...
	private boolean hasCorrectArmorOn(ArmorMaterial material,  PlayerEntity player){
		for(ItemStack armorStack: player.getInventory().armor) {
			if(!(armorStack.getItem() instanceof net.minecraft.item.ArmorItem)) {
				return false;
			}
		}
	...
	public ArmorItem(ArmorMaterial material, ArmorSlot slot, Settings settings) {
